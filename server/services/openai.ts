  ...
  async generateCommentAnalysis(videoData: VideoData): Promise<VideoAnalysis> {
    ...
    try {
      ...
      if (commentsToAnalyze.length === 0) {
        console.log("No comments available for analysis, returning simplified response");
        return {
      ...
      try {
        ...
        if (!analysis.sentimentStats || !analysis.keyPoints || !analysis.comprehensive) {
        ...
        throw new Error(`Failed to parse analysis: ${parseError?.message || 'JSON parsing error'}`);
      }
    } catch (error: any) {
      ...
      if (error.status === 429 || (error.error && error.error.type === "insufficient_quota")) {
        ...
        errorMessage = "401 Invalid API key. Please check your OpenAI API key and try again.";
      } else if (error.message.includes("timed out")) {
        errorMessage = "The request to OpenAI API timed out. Please try again later.";
      } else {
